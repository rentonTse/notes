...
from photutils import EPSFStar, EPSFStars, LinkedEPSFStar
from photutils import EPSFModel
from photutils.centroids import centroid_epsf
...
class EPSFBuilder:
    ...
    def _smooth_epsf(self, epsf_data):
        ...
        if self.smoothing_kernel is None:
            ...
            kernel = np.array(
                [[0.0173913,  0.03478261, 0.04347826, 0.03478261, 0.0173913 ],
                 [0.03478261, 0.07826087, 0.10434783, 0.07826087, 0.03478261],
                 [0.04347826, 0.10434783, 0.13043478, 0.10434783, 0.04347826],
                 [0.03478261, 0.07826087, 0.10434783, 0.07826087, 0.03478261],
                 [0.0173913 , 0.03478261, 0.04347826, 0.03478261, 0.0173913 ]])         
...
cat_hdu=fits.getheader(r'F:\xzt\fits2\Cat_bulge_chip8_20241002120000.000000.fits')
cat_data=fits.getdata(r'F:\xzt\fits2\Cat_bulge_chip8_20241002120000.000000.fits')



imagehdu=fits.getheader(r'F:\xzt\fits1\Img_bulge_chip8_20241002120000.000000.fits')
imagedata=fits.getdata(r'F:\xzt\fits1\Img_bulge_chip8_20241002120000.000000.fits')
imagedata=imagedata.astype(np.float64)
print(imagedata.shape)


data = imagedata[0:400,10000:10400]
print(data.shape)
...
# psfhdu=fits.getheader(r'C:\Users\ASUS\Desktop\chip_8_psf.fits')
# psfdata=fits.getdata(r'C:\Users\ASUS\Desktop\chip_8_psf.fits')
# psfdata=psfdata.astype(np.float64)
...
data_sl = np.array_split(data,nrows,axis=0)
a = data_sl


for i,data_i in enumerate(a):
    dataset = np.array_split(data_i,ncols,axis=1)

    for j,data_ij in enumerate(dataset):
        print((i*ncols+j+1),data_ij.shape)
    

        peaks_tbl = find_peaks(data_ij, threshold=5000.)  
        ...
        x = peaks_tbl['x_peak']  
        y = peaks_tbl['y_peak']  
        mask = ((x > hsize) & (x < (data_ij.shape[1] -1 - hsize)) &
                (y > hsize) & (y < (data_ij.shape[0] -1 - hsize)))  
        ...
        mean_val, median_val, std_val = sigma_clipped_stats(data_ij, sigma=3.)  
        data_ij -= median_val  

        # invert to nddata type because of the function extract_stars
        nddata = NDData(data=data_ij)
        ...
        epsf_builder = EPSFBuilder(oversampling=oversampling, shape=None,
                        smoothing_kernel='gaussian', recentering_func=centroid_com,
                        recentering_maxiters=20, fitter=EPSFFitter(), maxiters=10,
        ...
        epsfset=[[] for k in range(nrows*ncols)]

        for k in range(nrows*ncols):
            newmat=epsf.data
            epsfset[k].append(newmat)
...
# sys.stdout = open('fitted_stars.center_flat.log',mode = 'w',encoding='utf-8')
# print(fitted_stars.center_flat)
# sys.stdout.close()



# for [i,j] in fitted_stars.center_flat:
#     plt.scatter(i, j, color='', marker='o', edgecolors='g', s=20)

# cds_y = np.sort(cat_data, order='SE0Y')
# cds_y = cds_y
# indexs=[]
# for i in range(0,cds_y.shape[0]):
#         if cds_y[i][2] < 17:
#                 indexs.append(i)

# for (i,j,k) in cds_y[indexs]:
#     plt.scatter(i, j, color='', marker='o', edgecolors='r', s=20) 
# # plt.imshow(data, cmap='gray')
# # plt.colorbar()
# plt.show()
...
# ax = ax.ravel()
# for k in range(nrows*ncols):
#     norm = simple_norm(epsfset[k][0], 'log', percent=99.)
#     ax[k].imshow(epsfset[k][0], norm=norm, origin='lower', cmap='viridis')
#     ax_divider = make_axes_locatable(ax[k])
#     # Add an axes to the right of the main axes.
#     cax = ax_divider.append_axes("right", size="7%", pad="2%")
#     fig.colorbar(ax[k].imshow(epsfset[k][0], norm=norm, origin='lower', cmap='viridis'), cax=cax)
# plt.suptitle('ePSF')
# # plt.savefig(r'C:\Users\ASUS\Desktop\picture\{%ix%i}_parts_1k(g2_60k)_ePSF.png'%(nrows,ncols))
# plt.clf()
 


# m = IntegratedGaussianPRF(sigma=3,x_0=50, y_0=50, flux=8)
# yy, xx = np.mgrid[0:101, 0:101]
# norm_data = m(xx, yy)
# norm = simple_norm(norm_data,'log', percent=99.)
# # plt.imshow(norm_data, norm=norm,origin='lower', cmap='viridis')
...
# ax = ax.ravel()
# for k in range(nrows*ncols):
#     norm = simple_norm(epsfset[k][0]-norm_data, 'log', percent=99.)
#     ax[k].imshow(epsfset[k][0]-norm_data,norm=norm, origin='lower', cmap='viridis')
#     ax_divider = make_axes_locatable(ax[k])
#     # Add an axes to the right of the main axes.
#     cax = ax_divider.append_axes("right", size="7%", pad="2%")
#     fig.colorbar(ax[k].imshow(epsfset[k][0], norm=norm, origin='lower', cmap='viridis'), cax=cax)
# plt.suptitle('ePSF residues')
# # plt.savefig('/home/zhuanzhuan/xzt/pic/{%ix%i}_parts_1k(g2_60k)_ePSF_res.png'%(nrows,ncols))
# plt.clf()

X=np.zeros(101*nrows*ncols)
X=X.reshape(nrows*ncols,101)
...
print(epsfset[0][0].shape)
print(X.shape)




mean_X, median_X, std_X = sigma_clipped_stats(X, sigma=3.)
X_sigma =X - median_X
...
p_epsf = pca.transform(X_sigma)
print(p_epsf)

mean_epsf= X.mean(axis=0)
mean_epsf=mean_epsf.reshape(1,101)
...
plt.plot(mean_epsf[0,:],color='black',label='mean ePSF')
# plt.plot(norm_data[50,:], color='red',label='"true" ePSF')
plt.plot(p_epsf[0,:],color='green',label='PCA ePSF')
plt.legend(loc=1)
# # plt.savefig('/home/zhuanzhuan/xzt/pic/{%ix%i}_parts_1k(g2_60k)_curve.png'%(nrows,ncols))
